...
Please answer with either "CORRECT", "INCORRECT", or "UNVERIFIABLE" without any explanation.
YOUR JUDGEMENT:
"""
...
class PreciseQAEval:
    def __init__(self, model_path, TASKNAME):
        ...
        self.abtention_evaluator = 'gemini-2.5-flash-lite'
        self.halu_evaluator = 'gemini-2.5-flash-lite'

    def eval_abstention(self, evaluator):
        print("Start abstantion evaluation")
        abs_path = f'{self.output_path}/abstain_eval_raw.jsonl'
        abs_clean_path = f'{self.output_path}/abstain_eval_clean.jsonl'
        abstain_prompts = [
        ...
        if os.path.exists(abs_path):
            ...
            abstains_eval_model_is_gemini = evaluator.startswith("gemini")

            if abstains_eval_model_is_gemini:
                def run_abstain_generate(p):
                    return lm.call_gemini_api(p, model=evaluator)
            else:
                def run_abstain_generate(p):
                    return lm.call_vllm_api(p, model=evaluator)
                
            abstains_eval_raw = thread_map(
                lambda p: run_abstain_generate(p),
                abstain_prompts,
                max_workers=2,
        ...
        eval_utils.save_eval_raw(abstains_eval, abs_clean_path)
        
        return abstains_eval

    def judge_hallucination(self, evaluator):
        print("Starting Hallucination Evaluation")
        halu_file_path = f'{self.output_path}/hallu_eval_raw.jsonl'
        halu_clean_path = f'{self.output_path}/hallu_eval_clean.jsonl'

        if os.path.exists(halu_file_path):
            # read from jsonl halu_file_path
            with open(halu_file_path, "r") as f:
                halu_eval_raw = [line.strip() for line in f]
        else:
            halu_prompts = [
            ...
            if evaluator == "meta-llama/Llama-3.1-70B-Instruct":
            ...
            elif evaluator == "Qwen/Qwen3-30B-A3B-Instruct-2507":
                halu_eval_raw = thread_map(
                    lambda p: lm.generate(p, evaluator),
                    halu_prompts,
                    max_workers=32,
                ...
                halu_eval_raw = thread_map(
                    lambda p: lm.call_gemini_api(p, model=evaluator),
                    halu_prompts,
                    max_workers=2,
                    desc=f"using {evaluator} for hallucination judgement")
            else:
                raise ValueError(f"Invalid evaluator: {evaluator}")

            eval_utils.save_eval_raw(halu_eval_raw, halu_file_path)
        
        halu_eval = eval_utils.check_hallucination(halu_eval_raw)
        eval_utils.save_eval_raw(halu_eval, halu_clean_path)
        return halu_eval

    def process_res(self, abstantion_res, halu_eval_raw):
        abstantion_res = [x['is_abstaining'] for x in abstantion_res]
        halu_res = []
        for txt in halu_eval_raw:
            ...
            halu_res.append(hallucinated_judge)
        return abstantion_res, halu_res

    def run_eval(self):
        abstantion_res = self.eval_abstention(self.abtention_evaluator)
        halu_res = self.judge_hallucination(self.halu_evaluator)
        print("Top 10 abstention results: ", abstantion_res[:10])
        print("Top 10 hallucination results: ", halu_res[:10])
        
        abstantion_res, halu_res = self.process_res(abstantion_res, halu_res)

        not_abstained = sum([1 for x in abstantion_res if x == False])
        if not_abstained == 0:
            hallu_rate_not_abstain = 0
        else:
            hallu_rate_not_abstain = sum([1 for is_abstaining, is_hallucinated in zip(abstantion_res, halu_res) \
        ...
        correct_rate = sum([1 for is_hallucinated in halu_res if is_hallucinated == False])/len(halu_res)
        ...
        res = {
            ...
            'halu_res': halu_res,
            'abstantion_raw_generation' : abstantion_res,
            'is_hallucinated_raw_generation' : halu_res,
        }
...
if __name__ == '__main__':
    ...
    if args.do_generate_prompt:
        # 1. Generate QA pairs
        QA_OUTPUT_PATH = f'data/precise_qa/save/qa_{args.wiki_src}_{args.mode}.jsonl'\
