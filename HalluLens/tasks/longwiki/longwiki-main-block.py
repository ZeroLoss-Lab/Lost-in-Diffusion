...
def run_eval(args):
    ...
    output_folder = Path(f'output/{TASKNAME}-block_32-{args.exp_mode}/{model_name}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='gemini-2.5-pro', help='model that is being "TESTED"')
    parser.add_argument('--q_generator', type=str, default='gemini-2.5-pro', help='model that is used for question generation')

    parser.add_argument('--claim_extractor', type=str, default='gemini-2.5-flash-lite', help='model that is used for claim extraction')
    parser.add_argument('--abstain_evaluator', type=str, default="gemini-2.5-flash-lite", help='model that is used for abstantion evaluation')
    parser.add_argument('--verifier', type=str, default='gemini-2.5-flash-lite', help='model that is used for final verification')
    ...
    base_path = os.path.dirname(os.path.abspath(__name__))
    model_name = args.model.split("/")[-1]
    QA_OUTPUT_PATH = f"data/longwiki/save/longwiki.jsonl"
