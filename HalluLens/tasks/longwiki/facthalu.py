...
encoding = AutoTokenizer.from_pretrained('Qwen/Qwen2.5-7B-Instruct', trust_remote_code=True)
...
class FactHalu:
    def __init__(
            ...
            output_csv: str,
            abstain_evaluator: str = "gemini-2.5-flash-lite",
            refusal_evaluator: str ='gemini-2.5-flash-lite',
            claim_extractor: str = "gemini-2.5-flash-lite",
            verifier: str = 'gemini-2.5-flash-lite',
            k: int = 32,
        ...
        # print("Using abstain evaluator:", self.abstain_evaluator)
        # print("Using refusal evaluator:", self.refusal_evaluator)
        # print("Using claim extractor:", self.claim_extractor)
        # print("Using verifier:", self.verifier)
        # exit(-1)
        ...
        with open(self.generations_file_path, "r") as f:
            for line in f:
                ...
                if generation.generation.strip() == "":
                    generation.generation = "No answer provided."
...
def make_claim_extraction_prompts(generation: Generation, claim_extractor="gemini-2.5-flash-lite"):
    """
    ...
    elif claim_extractor == "gemini-2.5-flash-lite":
        for i, sentence in list(enumerate(sentences_text)):
            if len(sentence) < 5:
                continue
            ...
            if prompt_len > 3500:
                ...
                encoded_prompt = encoding.encode(prompt_text)
                if len(encoded_prompt) > 3500:
                    print(f"Warning: Prompt too long ({len(encoded_prompt)} tokens). Truncating to 3500 tokens.")
                    # Decode back the first 3500 tokens to get the truncated text
                    prompt_text = encoding.decode(encoded_prompt[:3500])
