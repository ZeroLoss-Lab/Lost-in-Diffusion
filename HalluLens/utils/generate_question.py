...
class WikiQA:
    def __init__(self, q_generator_path, task):
        ...
        if "gemini" in q_generator_path.lower() or "gpt" in q_generator_path.lower():
            print(f"Detected API model [{q_generator_path}]. Using fallback tokenizer for document splitting.")
            tokenizer_name = "Qwen/Qwen2.5-7B-Instruct"
        else:
            tokenizer_name = q_generator_path
        self.encoding = AutoTokenizer.from_pretrained(tokenizer_name, trust_remote_code=True)
    ...
    def per_bin_generation_batch(self, wiki_data, output_path, N):
        ...
        provider_is_gemini = self.q_generator.startswith("gemini")


        if provider_is_gemini:
            def run_question_generate(p):
                return lm.call_gemini_api(p, model=self.q_generator, temperature=0.7, top_p=0.9)
        else:
            def run_question_generate(p):
                return lm.call_vllm_api(p, model=self.q_generator, temperature=0.7, top_p=0.9)

        results = thread_map(lambda p: run_question_generate(p),
                                Q_MAKING_PROMPTS,
                                max_workers=2,
        ...
        #                             max_workers=50,
        #                             desc=f"using {self.q_generator}")
        ans_results = thread_map(lambda p: run_question_generate(p),
                                    prompts_answerability,
                                    max_workers=3,
                                    desc=f"using {self.q_generator} for answerability")
        filter_count = 0
...
def precise_QA_generation_run_batch(
    ...
    for bin in range(low_level,high_level):
        ...
        actal_needed = int(per_level_count * 1.25) # 25% more to ensure enough after filtering

        wiki_data = wiki_data[:actal_needed]
...
def longform_QA_generation_run_batch(
    ...
    wiki_data_all = pd.read_json(wiki_input_path, orient='records', lines=True)
    non_exist_titles_file = "data/wiki_data/doc_goodwiki_not_exist_titles.txt"
    if os.path.exists(non_exist_titles_file):
    ...
    for bin in range(low_level,high_level):
        ...
        wiki_data = level_wiki.to_dict(orient='records')
        random.shuffle(wiki_data)
        wiki_data = wiki_data[:per_level_count+50] #todo 100
