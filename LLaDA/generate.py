...
def generate(model, prompt, attention_mask=None, steps=128, gen_length=128, block_length=128, temperature=0.,
             cfg_scale=0., remasking='low_confidence', mask_id=126336, logits_eos_inf=False, confidence_eos_eot_inf=False, tokenizer=None):
    '''
    Args:
    ...
    for num_block in range(num_blocks):
        ...
        for i in range(steps):
            ...
            num_step = i + num_block * steps + 1
            output_steps = [16, 32, 64, 128, 256, 512, 1024]
            if num_step in output_steps:
                print("step {}: {}".format(i + num_block * steps + 1, answer), end="\n")
                sleep(1)
                print("step {}: {}".format(i + num_block * steps + 1, answer), end="\n")
                sleep(1)
